 ...
 function __construct(){
   ...
   function login($idioma=null)
   {
      ...
      if(!isset($_POST['maillogin'])){   //   Si no recibimos ningún valor proveniente del formulario, significa que el usuario recién ingresa.   
         $this->load->view('php/login');      //   Por lo tanto le presentamos la pantalla del formulario de ingreso.
      }
         ...
         if(($this->form_validation->run()==FALSE)){            //   Verificamos si el usuario superó la validación
            $this->load->view('php/login');                     //   En caso que no, volvemos a presentar la pantalla de login
         }
           ...
           $this->load->view('registros/index4'); 
                ...
                if($ExisteUsuarioyPassoword->privilegio == 2){
                 ...
                 if($ExisteUsuarioyPassoword->privilegio == 3){
                   ...
                   if($ExisteUsuarioyPassoword->privilegio == 5){
               ...
               $this->load->view('php/login',$data);   //   Lo regresamos a la pantalla de login y pasamos como parámetro el mensaje de error a presentar en pantalla
            }
         }
      }
   }
